        ...
        {faqs.length > 0 ? (
          <Accordion type="single" collapsible className="w-full space-y-4">
            {faqs.map((faq: FAQ, index: number) => (
              <AccordionItem value={`item-${index + 1}`} key={faq.id} className="bg-card shadow-md rounded-lg">
                <AccordionTrigger className="px-6 py-4 text-left text-lg font-medium hover:no-underline text-foreground">
                  {faq.question}
                </AccordionTrigger>
