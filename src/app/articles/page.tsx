...
import { getAllArticles } from '@/lib/data'; // Renamed from getAllBlogPosts
import ArticleCard from '@/components/articles/article-card'; // Renamed from BlogCard and path updated
import NewsletterSection from '@/components/sections/newsletter-section';
import { Input } from '@/components/ui/input';
import { Search } from 'lucide-react';

export const metadata: Metadata = {
  title: 'Aura Articles - Wellness & Mindfulness', // Updated title
  description: 'Explore articles on wellness, mindfulness, meditation, and natural living from Aura.',
};

export default function ArticlesPage() { // Renamed from BlogPage
  const posts = getAllArticles(); // Renamed from getAllBlogPosts
      ...
      <header className="py-16 md:py-24 bg-gradient-to-b from-background to-secondary/30 text-center">
        <div className="container mx-auto max-w-screen-md px-4 sm:px-6 lg:px-8">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-foreground mb-4">
            The Aura <span className="text-primary">Journal</span> {/* Keeping Journal for now, can be changed */}
          </h1>
      ...
      <main className="container mx-auto max-w-screen-lg px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        {posts.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map((post) => (
              <ArticleCard key={post.id} post={post} /> // Using ArticleCard
            ))}
          </div>
        ) : (
